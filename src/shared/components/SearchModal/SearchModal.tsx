...
import { fetchTMDBMovieSearch, TMDBMovie, getTMDBImageUrl } from "shared/services/tmdbApi";
...
/**
 * EN: Global quick search modal (triggered via Ctrl+K or header search icon).
 * Searches both Cybersoft movies and TMDB global database in real-time.
 * VI: Modal tìm kiếm nhanh toàn cục (kích hoạt bằng Ctrl+K hoặc icon tìm kiếm trên header).
 * Tìm kiếm đồng thời phim Cybersoft và kho phim TMDB theo thời gian thực.
 */
const SearchModal: FC<SearchModalProps> = ({ open, onClose }) => {
  ...
  // Fetch Cybersoft movie catalog once on open for fast client-side filtering
  useEffect(() => {
    if (!open) {
      setSearchTerm("");
      setTmdbMovies([]);
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        if (results && (results as any).id) {
          ...
          // Fetch array of search results via TMDB API endpoint
  ...
  // Filter Cybersoft movies matching search query
